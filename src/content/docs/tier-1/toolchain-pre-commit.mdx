...
皆さんはすでに[フォーマッターとリンター](https://extra.utcode.net/tier-1/style-formatter-and-linter/)について学んでいるので、コミットされた後のソースコードが、正しいフォーマットで、かつ品質や潜在的なバグをチェックされたものであることを保証するために、コミットする前にフォーマッターとリンターを実行しましょう。
